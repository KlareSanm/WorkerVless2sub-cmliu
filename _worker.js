...
function surge(content, url, path) {
  ...
  输出内容 =
    `#!MANAGED-CONFIG ${url.href} interval=86400 strict=false` +
...
function getRandomProxyByMatch(CC, socks5Data) {
  ...
  let filteredProxies = socks5Data.filter((proxy) =>
    proxy.toLowerCase().endsWith(`#${lowerCaseMatch}`)
  );

  // 如果没有匹配的代理，尝试匹配 "US"
  if (filteredProxies.length === 0) {
    filteredProxies = socks5Data.filter((proxy) =>
      proxy.toLowerCase().endsWith(`#us`)
    );
  }
...
function generateFakeInfo(content, userID, hostName) {
  content = content
    .replace(new RegExp(userID, "g"), fakeUserID)
    .replace(new RegExp(hostName, "g"), fakeHostName);
  return content;
}
...
function 生成动态UUID(密钥) {
  ...
  function 生成UUID(基础字符串) {
    ...
    return crypto.subtle.digest("SHA-256", 哈希缓冲区).then((哈希) => {
      ...
      const 十六进制哈希 = 哈希数组
        .map((b) => b.toString(16).padStart(2, "0"))
        .join("");
      return `${十六进制哈希.substr(0, 8)}-${十六进制哈希.substr(
        8,
        4
      )}-4${十六进制哈希.substr(13, 3)}-${(
        (parseInt(十六进制哈希.substr(16, 2), 16) & 0x3f) |
        0x80
      ).toString(16)}${十六进制哈希.substr(18, 2)}-${十六进制哈希.substr(
        20,
        12
      )}`;
    });
  ...
  const 到期时间字符串 = `到期时间(UTC): ${到期时间UTC
    .toISOString()
    .slice(0, 19)
    .replace("T", " ")} (UTC+8): ${结束时间
    .toISOString()
    .slice(0, 19)
    .replace("T", " ")}\n`;
        ...
        订阅链接.map((apiUrl) =>
          fetch(apiUrl, {
            ...
            headers: {
              Accept: "text/html,application/xhtml+xml,application/xml;",
              "User-Agent": `\u0076\u0032\u0072\u0061\u0079\u004e\u002f${
                FileName + atob("IGNtbGl1L1dvcmtlclZsZXNzMnN1Yg==")
              }`,
            },
...
export default {
  async fetch(request, env) {
    ...
    alpn = env.ALPN || alpn;
    let UD = Math.floor(
      (((timestamp - Date.now()) / timestamp) * 99 * 1099511627776 * 1024) / 2
    );
    if (env.UA)
      MamaJustKilledAMan = MamaJustKilledAMan.concat(await 整理(env.UA));
    ...
    fakeHostName =
      fakeUserIDMD5.slice(6, 9) + "." + fakeUserIDMD5.slice(13, 19) + ".xyz";
    ...
    if (
      快速订阅访问入口.length > 0 &&
      快速订阅访问入口.some((token) => url.pathname.includes(token))
    ) {
      host = "null";
      if (env.HOST) {
      ...
      隧道版本作者 =
        url.searchParams.get(atob("ZWRnZXR1bm5lbA==")) ||
        url.searchParams.get(atob("ZXBlaXVz")) ||
        隧道版本作者;
      ...
      if (url.searchParams.has("alterid")) {
        ...
        url.searchParams.has(atob("ZWRnZXR1bm5lbA==")) ||
        url.searchParams.has("uuid")
      ) {
        协议类型 = atob("VkxFU1M=");
      } else if (
        url.searchParams.has(atob("ZXBlaXVz")) ||
        url.searchParams.has("password") ||
        url.searchParams.has("pw")
      ) {
        协议类型 = atob("VHJvamFu");
      }
    ...
    if (
      ...
      host.toLowerCase().includes("worker") ||
      host.toLowerCase().includes("trycloudflare")
    )
      noTLS = "true";
    ...
    if (
      ...
      MamaJustKilledAMan.some(
        (PutAGunAgainstHisHeadPulledMyTriggerNowHesDead) =>
          userAgent.includes(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead)
      ) &&
      ...
      (userAgent.includes("clash") ||
        (format === "clash" && !userAgent.includes("subconverter"))) &&
      !userAgent.includes("nekobox") &&
      !userAgent.includes("cf-workers-sub")
    ) {
      subConverterUrl = `https://${subConverter}/sub?target=clash&url=${encodeURIComponent(
        subConverterUrl
      ...
      (userAgent.includes("sing-box") ||
        ...
        (format === "singbox" && !userAgent.includes("subconverter"))) &&
      !userAgent.includes("cf-workers-sub")
    ) {
      subConverterUrl = `https://${subConverter}/sub?target=singbox&url=${encodeURIComponent(
        subConverterUrl
      )}&insert=false&config=${encodeURIComponent(
        subConfig
      )}&emoji=true&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true`;
        ...
        notlsresponseBody = uniqueAddressesnotls
          .map((address) => {
            ...
            if (
              ...
              if (socks5Data) {
                const socks5 = getRandomProxyByMatch(
                  lowerAddressid,
                  socks5Data
                );
                path = `/${socks5}`;
                ...
                for (let item of 匹配PROXYIP) {
                  if (
                    item.includes("#") &&
                    item.split("#")[1] &&
                    lowerAddressid.includes(item.split("#")[1].toLowerCase())
                  ) {
            ...
            if (协议类型 == "VMess") {
              const vmessLink = `vmess://${utf8ToBase64(
                `{"v":"2","ps":"${
                  addressid + EndPS
                }","add":"${address}","port":"${port}","id":"${uuid}","aid":"${额外ID}","scy":"${加密方式}","net":"ws","type":"${type}","host":"${host}","path":"${path}","tls":"","sni":"","alpn":"${encodeURIComponent(
                  alpn
                )}","fp":""}`
              )}`;
              return vmessLink;
            } else {
              const 维列斯Link = `${atob("dmxlc3M6Ly8=") + uuid}@${address}:${
                port +
                atob("P2VuY3J5cHRpb249bm9uZSZzZWN1cml0eT0mdHlwZT0=") +
                type
              }&host=${host}&path=${encodeURIComponent(
                path
      ...
      const responseBody = uniqueAddresses
        .map((address) => {
          ...
          if (临时中转域名.length > 0 && host.includes(".workers.dev")) {
            最终路径 = `/${host}${path}`;
            伪装域名 =
              临时中转域名[Math.floor(Math.random() * 临时中转域名.length)];
            节点备注 =
              EndPS +
              atob(
                "IOW3suWQr+eUqOS4tOaXtuWfn+WQjeS4rei9rOacjeWKoe+8jOivt+WwveW/q+e7keWumuiHquWumuS5ieWfn++8gQ=="
              );
            sni = 伪装域名;
          }
          ...
          if (协议类型 == "VMess") {
            const vmessLink = `vmess://${utf8ToBase64(
              `{"v":"2","ps":"${
                addressid + 节点备注
              }","add":"${address}","port":"${port}","id":"${uuid}","aid":"${额外ID}","scy":"${加密方式}","net":"ws","type":"${type}","host":"${伪装域名}","path":"${最终路径}","tls":"tls","sni":"${sni}","alpn":"${encodeURIComponent(
                alpn
              )}","fp":""}`
            )}`;
            return vmessLink;
          } else if (协议类型 == atob("VHJvamFu")) {
            const 特洛伊Link = `${atob("dHJvamFuOi8v") + uuid}@${address}:${
              port + atob("P3NlY3VyaXR5PXRscyZzbmk9") + sni
            }&alpn=${encodeURIComponent(
              alpn
            )}&fp=randomized&type=${type}&host=${伪装域名}&path=${encodeURIComponent(
              最终路径
            ...
            const 维列斯Link = `${atob("dmxlc3M6Ly8=") + uuid}@${address}:${
              port +
              atob("P2VuY3J5cHRpb249bm9uZSZzZWN1cml0eT10bHMmc25pPQ==") +
              sni
            }&alpn=${encodeURIComponent(
              alpn
            )}&fp=random&type=${type}&host=${伪装域名}&path=${encodeURIComponent(
              最终路径
            )}#${encodeURIComponent(addressid + 节点备注)}`;
            return 维列斯Link;
          }
      ...
      if (
        ...
        (userAgent.includes("surge") ||
          (format === "surge" && !userAgent.includes("subconverter"))) &&
        !userAgent.includes("cf-workers-sub")
      ) {
        const 特洛伊Links = combinedContent.split("\n");
        const 特洛伊LinksJ8 = generateFakeInfo(
          特洛伊Links.join("|"),
          uuid,
          host
        );
        subConverterUrl = `https://${subConverter}/sub?target=surge&ver=4&url=${encodeURIComponent(
          特洛伊LinksJ8
        )}&insert=false&config=${encodeURIComponent(
          subConfig
        )}&emoji=true&list=false&xudp=false&udp=false&tfo=false&expand=true&scv=true&fdn=false`;
      } else {
        let base64Response;
        try {
    ...
    try {
      ...
      if (!subConverterResponse.ok) {
        throw new Error(
          `Error fetching subConverterUrl: ${subConverterResponse.status} ${subConverterResponse.statusText}`
        );
      }
        ...
        "aHR0cHM6Ly9naXRodWIuY29tL2NtbGl1L1dvcmtlclZsZXNzMnN1Yg=="
			...
			<div class="container">
				...
				<div class="input-group">
					...
					<input type="text" id="link" placeholder="${decodeURIComponent(
            atob(
              "JUU4JUFGJUI3JUU4JUJFJTkzJUU1JTg1JUE1JTIwVk1lc3MlMjAlMkYlMjBWTEVTUyUyMCUyRiUyMFRyb2phbiUyMCVFOSU5MyVCRSVFNiU4RSVBNQ=="
            )
          )}">
				</div>
